...
def fitness(person1, person2, individual):
    ...
    output = zip(individual[0], person1) # task sequence
    robot_assignment = zip(individual[0], person2)  # robot assignment for each task
...
def genetic_algorithm(person1, person2, safety_distance, process_duration, population_size=2000, generations=2000):
    # Run the genetic algorithm
    population = [generate_individual(person1, person2) for _ in range(population_size)]
    for _ in range(generations):
        fitness_values = [fitness(person1, person2, individual) for individual in population]
        ...
        new_population = []
        individual_1 = fittest_individual[:len(person1)]
        individual_2 = fittest_individual[len(person1):]
        while len(new_population) < population_size:
            parent1, parent2 = random.sample(population, 2)
            offspring1, offspring2 = crossover(parent1, parent2)
            new_population.extend([mutate_individual(offspring1), mutate_individual(offspring2)])
        population = new_population
    return fittest_individual, fittest_fitness, individual_1, individual_2
...
stations = [0, 10, 20, 30, 40, 50]  # Station coordinates
